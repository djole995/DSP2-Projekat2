...
void bicubicInterpolate(const uchar input[], int xSize, int ySize, uchar output[], int newXSize, int newYSize)
{
	...
	for (int i = 0; i < newYSize; i++)
	{
		...
		m[1] = /*(int) floor(yPos)*/m[0]+1;
		m[2] = /*((int)yPos == 0) ? 1 : */(yPos < ySize-2) ? m[1]+1 : ySize-2;
		m[3] = /*(yPos < ySize-2) ? m[2] + 1 : ySize-1*/m[2]+1;
		...
		for (int j = 0; j < newXSize; j++)
		{
			...
			n[1] = /*(int)floor(xPos)*/n[0]+1;
			n[2] = /*((int)xPos == 0) ? 1 :*/ (xPos < xSize - 2) ? n[1]+1 : xSize - 2;
			n[3] = /*(xPos < xSize - 2) ? n[2] + 1 : xSize - 1*/n[2]+1;
	...
	for (int i = 0; i < newYSize/2; i++)
	{
		...
		m[0] = (yPos > 1) ? (int)floor(yPos) - 1 : 0;
		m[1] = /*(int) floor(yPos)*/m[0] + 1;
		m[2] = /*((int)yPos == 0) ? 1 : */(yPos < ySize/2 - 2) ? m[1] + 1 : ySize/2 - 2;
		m[3] = /*(yPos < ySize-2) ? m[2] + 1 : ySize-1*/m[2] + 1;
		...
		for (int j = 0; j < newXSize/2; j++)
		{
			...
			n[0] = (xPos > 1) ? (int)floor(xPos) - 1 : 0;
			n[1] = /*(int)floor(xPos)*/n[0] + 1;
			n[2] = /*((int)xPos == 0) ? 1 :*/ (xPos < xSize/2 - 2) ? n[1] + 1 : xSize/2 - 2;
			n[3] = /*(xPos < xSize - 2) ? n[2] + 1 : xSize - 1*/n[2] + 1;
